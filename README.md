...
- 首次搜索一个文件时需**按需解析**（无预建索引，这是设计选择）。解析用 `get_text("words")` 模式快速重建行（不做逐字符解码），33MB / 369 页年报约 **1 秒**；解析结果在会话内存缓存，后续搜索更快（约 0.3s）。
- **简繁归一化**：内置 OpenCC，简体查询/引文可直接命中繁体 PDF（反之亦然）；cite 对跨行空格差异不敏感。

## 已知限制

- **表格里的"键值对"数字召回较弱**：若关键数字是表格单元格（如业绩概览里「460 萬輛」与标签「快報銷量」分属两个单元格），段落级检索难以把数字和标签连起来，可能漏召回。已加"页上下文微加成"缓解密集表格，但稀疏键值对表格仍需后续的"表格感知合并"才能根治。临时可用「数字 + 单位」或表格标题（如「業績概覽」）等词检索，或直接翻页定位。
